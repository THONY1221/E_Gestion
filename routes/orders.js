...
async function logStockMovement(
  ...
  try {
    ...
    if (!validMovementTypes.includes(movementType)) {
      ...
      if (!validMovementTypes.includes(movementType)) {
        console.warn(
          `Skipping stock movement log due to invalid type: ${movementType}`
        );
    ...
    console.log(
      `Stock movement logged: Product ${productId}, Warehouse ${warehouseId}, Qty ${signedQuantity}, Type ${movementType}, Ref ${referenceType}:${referenceId}`
    );
  } catch (error) {
...
async function updateStock(
  ...
  if (isNaN(qtyChange) || !productId || !warehouseId) {
    console.warn(
      `Invalid data for stock update: Product ${productId}, Warehouse ${warehouseId}, Change ${quantityChange}. Skipping update.`
    );
    return false;
  }
  ...
  if (movementType === "transfer_in" && relatedWarehouseId) {
    try {
      ...
      if (sourcePriceRows.length > 0) {
        ...
        console.log(
          `Récupération des prix depuis l'entrepôt source ${relatedWarehouseId} pour produit ${productId}: Achat=${purchasePrice}, Vente=${salesPrice}`
        );
      } else {
        console.warn(
          `Détails produit non trouvés dans l'entrepôt source ${relatedWarehouseId} pour produit ${productId} lors du transfert.`
        );
        // Conserver les prix par défaut à 0 si non trouvé
      }
    } catch (priceError) {
      console.error(
  ...
  try {
    ...
    if (upsertResult.affectedRows > 0) {
      console.log(
        `Stock upserted: Product ${productId}, Warehouse ${warehouseId}, Change ${qtyChange}. Rows affected: ${upsertResult.affectedRows}`
      );
      ...
      if (qtyChange === 0) {
        console.log(
          `Stock update skipped for 0 quantity change: Product ${productId}, Warehouse ${warehouseId}`
        );
        // Log movement for 0 quantity? Probably not needed unless for audit.
        return true; // 0 change is technically successful.
      } else {
        console.warn(
          `Stock upsert reported 0 affected rows despite non-zero change (${qtyChange}): Product ${productId}, Warehouse ${warehouseId}. Result:`,
          upsertResult
        );
        // This suggests an unexpected issue.
...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    if (order_type === "sales" && user_id) {
      console.log("REQUÊTE DE VENTES POUR UN CLIENT:", {
        user_id,
        warehouse,
        order_type,
        order_status: order_status || "tous",
      });
    }
    if (order_type === "stock-transfer") {
      console.log("REQUÊTE DE TRANSFERTS DE STOCK:", {
        warehouse: selectedWarehouseId,
        view_type: view_type || "sent",
      });
    }
    if (order_type === "proforma") {
      console.log("REQUÊTE DE PROFORMAS:", {
        warehouse: warehouse,
        selectedWarehouseId: selectedWarehouseId,
        order_type: order_type,
        filtrage_actif: selectedWarehouseId ? "OUI" : "NON",
      });
    }
    ...
    // Debug log for proforma filtering
    if (order_type === "proforma") {
      console.log(`RÉSULTATS PROFORMAS - Total trouvé: ${orders.length}`);
      if (selectedWarehouseId) {
        const wrongWarehouseOrders = orders.filter(
          (o) => o.warehouse_id !== selectedWarehouseId
        );
        if (wrongWarehouseOrders.length > 0) {
          console.log(
            `⚠️ PROBLÈME: ${wrongWarehouseOrders.length} proformas ne correspondent pas au magasin ${selectedWarehouseId}:`
          );
          wrongWarehouseOrders.forEach((o) => {
            console.log(
              `  - Proforma #${o.id} appartient au magasin ${o.warehouse_id} (${o.warehouse_name})`
            );
          });
        } else {
          console.log(
            `✅ Toutes les proformas appartiennent bien au magasin ${selectedWarehouseId}`
          );
        }
      }
    }
    ...
    // Add debugging for status distribution if needed
    // if (orders.length > 0) { ... }
...
router.post("/", async (req, res) => {
  ...
  // Log détaillé des informations du fournisseur/client
  console.log("Création de commande - Informations user_id:", {
    user_id,
    order_type,
  });
  ...
  if (
    ...
    console.error(
      "ERREUR: user_id manquant pour une commande d'achat/retour d'achat"
    );
    return res.status(400).json({
      error:
  ...
  if ((order_type === "sales" || order_type === "sales_return") && !user_id) {
    console.error(
      "ERREUR: user_id manquant pour une commande de vente/retour de vente"
    );
    return res.status(400).json({
      error:
  ...
  try {
    ...
    if (!updateSuccess && order_type !== "proforma") {
      ...
      console.warn(
        `One or more stock updates failed for order ${orderId}. Continuing transaction, but stock might be inconsistent.`
      );
  ...
  try {
    ...
    // 3. Mise à jour complète de la commande (modification des articles, etc.)
      ...
      if (!oldStockReversalSuccess) {
        ...
        console.warn(
          `Un ou plusieurs ajustements de stock (annulation) ont échoué pour la commande ${orderId} lors de la mise à jour.`
        );
      ...
      if (!newStockUpdateSuccess) {
        ...
        console.warn(
          `Un ou plusieurs ajustements de stock (application) ont échoué pour la commande ${orderId} lors de la mise à jour.`
        );
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!reversalSuccess && orderInfo.order_type !== "proforma") {
      console.error(
        `Stock reversal failed during deletion of order ${orderId}. Rolling back transaction.`
      );
...
router.post("/:id/restore", async (req, res) => {
  ...
  try {
    ...
    if (!restoreSuccess && orderInfo.order_type !== "proforma") {
      console.error(
        `Stock restoration failed for order ${orderId}. Rolling back transaction.`
      );
...
router.get("/:id/invoice", async (req, res) => {
  try {
    ...
    if (orderRows.length === 0) {
      console.error(`Commande avec ID ${id} non trouvée`);
      return res.status(404).json({ error: "Commande non trouvée" });
    }
...
router.get("/available-for-return", async (req, res) => {
  ...
  try {
    ...
    if (isNaN(userId) || userId <= 0) {
      console.error("Paramètre user_id invalide:", userIdRaw);
      return res.status(400).json({
    ...
    if (isNaN(warehouseId) || warehouseId <= 0) {
      console.error("Paramètre warehouse_id invalide:", warehouseIdRaw);
      return res.status(400).json({
        error:
    ...
    console.log(
      "Vérification de l'existence de l'utilisateur avec ID:",
      userId
    );
    ...
    if (userCheck.length === 0) {
      console.error(
        `Utilisateur avec ID ${userId} non trouvé dans la base de données`
      );
    ...
    console.log(
      "Vérification de l'existence de l'entrepôt avec ID:",
      warehouseId
    );
    ...
    if (warehouseCheck.length === 0) {
      console.error(
        `Entrepôt avec ID ${warehouseId} non trouvé dans la base de données`
      );
      return res.status(404).json({
...
router.post("/:id/convert-to-sale", async (req, res) => {
  ...
  try {
    console.log(`Conversion de la proforma ${proformaId} en vente`);
    await connection.beginTransaction();
    ...
    for (const item of proformaItems) {
      ...
      if (updateResult.affectedRows === 0) {
        console.warn(
          `Stock (conversion proforma) non mis à jour pour produit ${item.product_id}. Quantité: ${quantity}. Lignes affectées: ${updateResult.affectedRows}`
        );
        // throw new Error(`Impossible de mettre à jour le stock pour le produit ID ${item.product_id} lors de la conversion.`);
      } else {
        console.log(
          `Stock (conversion proforma) mis à jour pour produit ${item.product_id}: -${quantity}`
        );
      }
    }
...
// Route pour générer un PDF de facture à partir d'un template HTML
router.post("/generate-sale-invoice-pdf", async (req, res) => {
  console.log("API endpoint /generate-sale-invoice-pdf called");
  try {
    const { htmlContent, fileName } = req.body;

    console.log("Request received - fileName:", fileName);
    console.log(
      "HTML content length:",
      htmlContent ? htmlContent.length : "undefined"
    );

    if (!htmlContent) {
      console.log("Error: HTML content is required but missing");
    ...
    console.log("Sending PDF response with size:", pdfBuffer.length, "bytes");
...
router.post("/generate-proforma-pdf", async (req, res) => {
  console.log("API endpoint /generate-proforma-pdf called");
  try {
    const { htmlContent, fileName } = req.body;
